...
public class ResultIntermediate extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (correctAnswers == totalQuestions && totalQuestions != 0) {
            imageView5.setImageResource(R.drawable.badge_intermediate); // optional trophy image
            ...
            badgeEditor.putBoolean("badge_intermediate_unlocked", true);
            badgeEditor.apply();
        ...
        int highestCorrectAnswers = sharedPreferences.getInt("highestCorrectAnswersIntermediate", 0);
        int highestTotalQuestions = sharedPreferences.getInt("highestTotalQuestionsIntermediate", 0);
...
// Check if the current score is higher, and only update if true
        if (correctAnswers > highestCorrectAnswers ||
            ...
            editor.putInt("highestCorrectAnswersIntermediate", correctAnswers);
            editor.putInt("highestTotalQuestionsIntermediate", totalQuestions);
            editor.apply();
        }
    ...
    private void setupButtonListeners(int correctAnswers, int wrongAnswers, String timeSpent) {
        backHomeButton.setOnClickListener(v -> {
            startActivity(new Intent(ResultIntermediate.this, MainActivity.class));
            finish();
        });
        ...
        reviewAnswer.setOnClickListener(v -> {
            if (getIntent().hasExtra("questions") &&
                ...
                Intent reviewIntent = new Intent(ResultIntermediate.this, ReviewAnswersActivityEasy.class);
